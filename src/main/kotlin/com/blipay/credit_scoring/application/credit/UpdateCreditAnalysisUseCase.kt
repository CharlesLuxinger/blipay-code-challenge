...
@Service
class UpdateCreditAnalysisUseCase(
    private val customerStore: CustomerStore,
    private val weatherPort: WeatherPort,
    private val clock: Clock,
) {
    fun execute(
        documentNumber: com.blipay.credit_scoring.domain.credit.entity.DocumentNumber,
        input: CreditAnalysisInput,
    ): AnalysisResult {
        val existing = customerStore.findByDocument(documentNumber) ?: throw CustomerNotFoundException()
        ...
        customerStore.update(
            StoredAnalysis(
                existing.updated(input.name, input.age, input.monthlyIncome, input.city, timestamp),
                ScoreRecord(
                    0,
                    documentNumber,
                    input.name,
